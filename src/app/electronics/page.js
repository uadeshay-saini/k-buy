...
import { fetchAllElectronicsProducts } from "@/provider/redux/productSlice";
...
useEffect( () => {
    (async () => {
        try {
            const fetchClothingProducts = await dispatch(fetchAllElectronicsProducts());
            if(fetchClothingProducts.payload.statusCode <=  200){
               if(fetchClothingProducts.payload.success){
                setElectronicsProducts(fetchClothingProducts.payload.data.products)
